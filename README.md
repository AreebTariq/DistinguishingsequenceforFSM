...
## General description
 ...
 FSMHSGen is a tool that generates all possible homing sequences and distinguishing sequences for a Finite State Machine (FSM), which can be partial, nondeterministric and nonobservable. The algorithm for obtaining such homing and distinguishing sequences is based on the algorithm published in [1]. It must be noted that the term all possible, refers to the maximal amount of homing and distinguishing sequences depending on the configuration parameters. Indeed, the implementation uses a modified version of the algorithm presented in [1]; the truncating rule #2 has been extended not only to consider an upper bound on the length of the input sequence, but also, the time spent for deriving the homing and distinguishing sequences, and the memory utilized for such purpose. More details on the configuration parameters, and homing and distinguishing sequence generation algorithm can be found later in this document. 
 ...
 As an output, FSMHSGen provides to the standard output (console) a list of all possible homing and distinguishing sequences. Each homing and distinguishing sequence is represented as dot separated input list, and finishing with a newline (\n) for example, “HS:3.1.5\n” or "DS: 4.2.6\n". Where the inputs correspond to the inputs in the “.fsm” input file.
 ...
 FSMHSGen also provides user the feature to control the initial states for the determination of the distinguishing sequences. This is done by selecting the value representing percentage (0-100) of the initial states from the overall all possible states of the machine.
...
### MaxMemSize_for_ds
 The parameter MaxMemSize_for_ds specifies the maximal amount of RAM memory (in MB) the tool can utilize. When the distinguishing sequence algorithm reaches the value of RAM specified by the MaxMemSize_for_ds parameter, a notice is produced and the tool terminates its execution. A value of 0 means unlimited. The default value is 0. Negative integers or other inputs will be ignored and a warning will be produced. The associated command line flag is “-mm_ds”
...
### MaxTime_for_ds
 The parameter MaxTime_for_ds specifies the maximal amount of time (in seconds) the generation of distinguishing sequences can take. When the distinguishing sequence algorithm reaches the time specified by the MaxTime_for_ds parameter, a notice is produced and the tool terminates its execution. A value of 0 means unlimited. The default value is 0. Negative integers or other inputs will be ignored and a warning will be produced. The associated command line flag is “-mt_ds” 
...
### MaxDSLength
 The parameter MaxDSLength specifies the maximal length of the potential distinguishing sequences. This parameter, as other maximum limits, when the sequences under investigation reach the value of MaxDSLength, a notice is produced and the program terminates its execution. A value of 0 means there is no limit for the length of the distinguishing sequences. The default value of MaxDSLength is 0. The associated command line flag is “-ml_ds”.
 
### Initial_state_percentage_for_ds
 The parameter Initial_state_percentage_for_ds specifies the percentage of initial states needed for the determination of the distinguishing sequences. A value of 100 means there is no limit for the initial states for processing of the distinguishing sequences. The default value of Initial_state_percentage_for_ds is 100. The associated command line flag is “-isp_ds”.
...
### DSLM
 The parameter DSLM specifies the need to compute and report the metric of the minimal length of a distinguishing sequence of the given FSM specification. The possible values for this parameter are true and false. The default value is false. The associated command line flag is “-dslm”. Note that this flag takes no value afterwards, its presence implies DSLM=true.
...
### Execution example: 
 ...
 _FSMHSGen -cf fsmhsgen.conf -ml 4 -ml_ds 3 inputFSM.fsm_
 ...
 \# Maximal amount of memory to use for distinguishing sequence
  
 MaxMemSize_for_ds=0 #(unlimited)

 \# Maximal time allowed to execute the experiment, 10s

 MaxTime=10
 
 \# Maximal time allowed to execute the experiment for distinguishing sequence, 10s
 ...
 \# Initial state percentage for distinguishing sequence
 
 Initial_state_percentage_for_ds = 100
